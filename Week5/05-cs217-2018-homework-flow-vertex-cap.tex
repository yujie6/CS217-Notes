...
\[
		t\le f\left( m t ^{\frac{1}{m}} \right) 
.\] 
Which is equivalent to find the largest $g = f^{-1}$ such that
\[
	  \forall m, mt ^{\frac{1}{m} } -	g\left( t \right)  \ge 0
.\] 
Thus $g$ cannot be polynomial, the maximum of $g$ could only be something like 
$c \ln\left( x \right) $. i.e.
\[
		h\left( t \right) = m t ^{\frac{1}{m}} - c\ln\left( t \right) , \forall t\ge 1,m\ge 1
.\] 
It easy to find the largest $c$ is  $1$. Hence the smallest $f$ is
$f\left( k \right) = e^{k}$
