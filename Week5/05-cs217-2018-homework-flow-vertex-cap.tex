...
\begin{proof}
	...
	Assume while $n=k$, there are $n \choose i$ vertex disjoint paths from $L_i$ to $L_{n-i}$.
	We want to proof while $k=n+1$ the conclusion is also right.
	We can divide the points between $L_i$ and $L_{n+1-i}$ into two set: $A=\{x|x[n+1]=0\}$ and $B=\{x|x[n+1]=1\}$. And according to induction assumption, there are $n \choose i$ vertex disjoint paths from $H_n[L_i]$ to $H_n[L_{n-i}]$
\end{proof}
