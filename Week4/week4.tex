...
\begin{proof}
		...
		since there is no need to sort the $t$ edges. In \textbf{Algorithm 1} 
		we just choose edges from large to small one by one, hence we can just use a 
		complex priority queue\footnote{ only supoort \texttt{insert} and \texttt{find\_max}, \texttt{delete\_max} actions, check this paper for detail: \texttt{On RAM priority queues}. }
		...
		priority queue could run, and currently there seems no better result.
\end{proof}
