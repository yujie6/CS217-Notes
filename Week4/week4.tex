...
\begin{proof}
		...
		.\] 
		And in the second round, we can obtain 4 blocks by apply MOM to $E_1,E_2$. Finally we have $2^{k}$ blocks $A_1,\ldots,A_{2^{k}}$, while
...
.\] 
And let $f\left( k \right)  = k 2^{k}$, assume that after $r$ iterations, we can decide $c^{*}$, we have 
\[
		f^{r-1}\left( 1 \right) <  m
.\] 
Which leads that $m \ge g^{r-1}\left( 1 \right)  $, in which $g\left( x \right) = 2^{x}$. Hence $r$ is the 
smallest integer such that 
$1 \le  \log \log\ldots\log m$ with $r$ log, the overall running time is 
